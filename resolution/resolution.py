...
#pooh = Sound("/home/ben/Documents/Landmark Acoustics/Clients/Hitchcock/VOT/C3Pooh2Log2_high135.29.wav")

#boo = Sound("/home/ben/Documents/Landmark Acoustics/Clients/Hitchcock/VOT/C3Boo13Log8_high95.59.wav")

#swsp = Sound("/home/ben/Documents/Landmark Acoustics/Technology Development/Optimal Overlap/three_swsp_syllables.wav")

#syll = Sound("/home/ben/Documents/Landmark Acoustics/Technology Development/Optimal Overlap/one_swsp_syllable.wav")

# du = Sound("/home/ben/Documents/Landmark Acoustics/Technology Development/Optimal Overlap/Deutschish/du.wav")
...
#             klaus[fft_size][time_index,offset] = kl_div(pdf_cache[time_index,],
#                                                         pdf_cache[-1,])
#         for time_index in range(offset, n):
#             cache_index = time_index % fft_size
#             pdf_cache[cache_index,] = as_pdf(Y[fft_size][time_index,])
#             klaus[fft_size][time_index,offset] = kl_div(pdf_cache[cache_index,],
#                                                         pdf_cache[(time_index - offset - 1) % fft_size,])

#     sums[fft_size] = np.nanmean(Y[fft_size],1)
#     ovlps = np.arange(1,fft_size+1)
#     maus[fft_size] = np.nansum(klaus[fft_size],0)/ovlps
#     saus[fft_size] = np.nansum(klaus[fft_size] * sums[fft_size][:,np.newaxis],0)/ovlps
...
def overlap_iteration(window_array:np.ndarray, overlap:float)->float:
    ...
    return (np.sqrt(np.nanmean(p)), kl_div(as_pdf(p), as_pdf(spk(window_array*x[-N:]))))

def my_summary(x:np.ndarray)->tuple:
    return (len(x),np.nanmean(x),np.nanstd(x)) if len(x) else (np.nan,np.nan,np.nan)
...
O = np.repeat(np.arange(40,0,-1)/40,250)
fft_sizes = 2**(np.arange(7)+6)
KL = np.zeros((len(O),len(fft_sizes)))
S = KL.copy()
KLsums = dict()
Ssums = dict()
BothSums = dict()
...
for n,X in sorted(KLsums.items()):
    plt.errorbar(100*(1-X['index']/n),X['mean kl'],X['kl sd']/np.sqrt(X['length']),label=n)
