...
class WDC:
    ...
    See Also
    --------
    soundresolutions.wial : The default divergence is the Weighted Independent Arc Length
    ...
    def __call__(self, sound : np.ndarray, overlap : float) -> Tuple[float, float, int]:
        r'''Finds the mean, variance, and sample size of the divergence function over the sound.

        Parameters
        ----------
        sound : np.ndarray
            The signal for processing. This should be read into memory in its entirety.
        overlap : float
            The proportional overlap between adjacent windows. Should be in [0.0,1.0).

        Returns
        -------
        sum : float
            The total of the divergences across the sound at the requested resolution.
        variance : float
        ...
        See Also
        --------
        soundresolutions.real_spk : Computes the real Fourier transforms for the spectra.
    ...
    def __call__(self, sound):
        ...
        for j, (k, wdc) in enumerate(self._wdcs.items()):
            for i, o in enumerate(self._overlaps):
                starts = np.array(np.unique(np.around(np.linspace(0, wdc.n_steps(o), self._n_starts, False))), dtype=int)
                starts.sort()
                d = 1.0 / len(starts)
                for s in starts:
                    m, v, n = wdc(sound[s:],o)
