...
from . import whole_steps
from . import euclidean_distance as EuD
from . import coefficient_of_alienation as CoA
...
class Thing:
    ...
    def __call__(self, X: np.ndarray, B: np.ndarray = None) -> np.ndarray:
        ...
        if B.shape != self.buffer_shape():
            estring = f'B should have shape {self.buffer_shape()}' \
                      ', use `buffer_shape()` to find it!'
...
class WeightedDivergenceComputer:
    """Computes weights, divergences, and weighted divergences.

    It takes spectrograms as inputs."""

    def __init__(self, fft_size: int, n_steps: int = 50, n_starts: int = 7):
    ...
    def update(self, spg: np.ndarray,
        ...
        for start in self._starts:
            for i, step in enumerate(self._steps):
                for x, y in zip(spg[start + step::step, ],
                                spg[start:-step:step, ]):
