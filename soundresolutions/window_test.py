...
class WindowTester:
    def __init__(self,
                 ...
                 steps : int = 59,
                 starts : int = 29,
                 windownames : List[str] = all_window_names) -> None:
...
if __name__ is '__main__':
    ...
    N = 10000
    sound_maker = lambda : np.random.normal(0,1,N)

    FS = 2 ** np.arange(6,12)
    
    window_tester = WindowTester(fft_sizes=FS)

    window_tester(sound_maker)

    print(f'{"Window":^18}'+''.join([f'{x:^10}' for x in FS])+f'{"Mean":^10}')
    O = window_tester.calculated_overlaps()
    for w, o, m in zip(sr.all_window_names, O, O.mean(1)):
        print(f'{w:>18}'+''.join([f'{x:10.1f}' for x in 100*o]) + f'{100*m:10.1f}')
